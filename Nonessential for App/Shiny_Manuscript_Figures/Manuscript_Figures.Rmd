---
title: 'Team X Manuscript Figures: Drinking Water'
...
human_setup <- human_v1 %>% # start with original data set
  mutate(size_h_f = factor(case_when(
    ...
    size.category == 0 ~ "Size Not Reported"), 
    levels = c("1nm < 100nm", "100nm < 1µm", "1µm < 100µm", "100µm < 1mm", "Size Not Reported"))) %>% #Renames for widget
...
```{r endpoints and organism groups, fig.width = 15}
library(viridis)
a <- human_setup %>% 
  filter(effect_h_f != "NA") %>%
  count(lvl1_h_f, vivo_h_f) %>% #how many obs of each organism group within each lvl1
  group_by(lvl1_h_f) %>%
  mutate(n_lvl1 = sum(n)) %>% #how many obs within each lvl1
  ungroup() %>%   
  ggplot(aes(x = fct_reorder(lvl1_h_f, n_lvl1), y = n, group = n, fill = vivo_h_f)) +
  geom_bar(stat = "identity", position = position_dodge2(preserve = "single"), color = "black", width = .5) +
  ...
  labs(y = "Number of Endpoints in Database", x = "Endpoint Category")
...
particle <- human_setup %>%
  group_by(size_h_f) %>%
  #count the number of data points in each size category
  mutate(size_count = n()) %>% 
  #percentage that report polymer
  mutate(poly_pct = (sum(poly_h_f != "Not Reported")/size_count)*100) %>% 
  #percentage that report shape
  mutate(shape_pct = (sum(shape_h_f != "Not Reported")/size_count)*100) %>%
  #percentage that report density
  replace_na(list(density.reported.estimated = "Not Reported")) %>%   
  mutate(density_f = as.factor(density.reported.estimated)) %>% 
  mutate(density_pct = (sum(density_f == "reported")/size_count)*100) %>%  
  #percentage that report charge
  mutate(charge_pct = (sum(!is.na(charge))/size_count)*100) %>%
  #percentage that report functional group
  mutate(fxngroup_pct = (sum(!is.na(functional.group))/size_count)*100) %>%
  #validate size
  mutate(sizevalid_f = as.factor(size.valid)) %>% 
  mutate(sizevalid_pct = (sum(sizevalid_f == "Y")/size_count)*100) %>% 
  #validate shape
  mutate(shapevalid_f = as.factor(shape.valid)) %>% 
  mutate(shapevalid_pct = (sum(shapevalid_f == "Y")/size_count)*100) %>% 
  #validate polymer
  mutate(polyvalid_f = as.factor(polymer.valid)) %>% 
  mutate(polyvalid_pct = (sum(polyvalid_f == "Y")/size_count)*100) %>% 
  ungroup() %>% 
  group_by(size_h_f, poly_pct, shape_pct, density_pct, charge_pct, fxngroup_pct, sizevalid_pct, shapevalid_pct, polyvalid_pct) %>% 
  summarise() %>% 
  rename("Polymer Type" = poly_pct, "Morphology" = shape_pct, "Density" = density_pct, "Charge" = charge_pct, "Functional Group" = fxngroup_pct, "Size Validated" = sizevalid_pct, "Morphology Validated" = shapevalid_pct, "Polymer Validated" = polyvalid_pct)
  

require(pheatmap)
require(RColorBrewer)  

# Make column 1 equal to the row name and remove data from column 1 and arrange size levels

All_data <- data.frame(particle, row.names = 1)%>% 
  arrange("1nm < 100nm", "100nm < 1µm", "1µm < 100µm", "100µm < 1mm", "Size Not Reported")

#Turn data into a matrix format

All_matrix <- data.matrix(All_data) 

color <- brewer.pal(n=8, name = "Blues")

Pretty_heatmap <- pheatmap(All_matrix,
        main = "Frequency (%) of Particle Characteristics Reported",
        fontsize = 12,
        treeheight_row = 0, treeheight_col = 0,
        cluster_rows = FALSE, cluster_cols = FALSE,
        display_numbers = TRUE,
        color = color,
        number_format = "%.0f", fontsize_number = 10, number_color = "black")
