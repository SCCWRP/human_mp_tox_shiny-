...
ug.Particle.drinkingWater = 5.65e-06 #calcaulated mean value from Hazimah
...
ggplot(Data2, aes(x = TapWater.particles.L.bioavailable, fill = aboveC)) + 
  ...
  geom_vline(xintercept = 10 * C.part.L, color = "green", linetype = "dashed") +
  geom_text(aes(x = C.part.L, y = 0.10), label = "Health Protective Concentration (NL)") +
  geom_text(aes(x = 10 * C.part.L, y = 0.07), label = "Response Level (10x NL)") +
  geom_text(aes(x = C.part.L, y = 0.09), label = paste(100*round(exceedance$Freq[1],4), "% Above NL")) +
  geom_text(aes(x = 10* C.part.L, y = 0.06), label = paste(100 * round(exceedance$Freq[2],4), "% Above RL")) +
  ...
  labs(title = "Microplastics in Tap Water Distribution (Particles/L; 1- 10 um)",
       ...
       caption = paste0("Based on Probability Density Functions in Mohamed Nor et al (2021); alpha = ",alpha, "; average mass of particle in drinking water = ", ug.Particle.drinkingWater, " ug/particle")
       )+
  theme_minimal(base_size = 15) +
  theme(
    plot.title = element_text(hjust = 0.5, face="bold"),
